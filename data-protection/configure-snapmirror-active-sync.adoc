...
NOTE: Ab ONTAP 9.19.1 RC unterstützt der autonome Ransomware-Schutz mit künstlicher Intelligenz (ARP/AI) primäre Lese-/Schreibspeichereinheiten in SnapMirror active sync SAN-Beziehungen auf ASA r2-Systemen. ARP/AI läuft auf den aktiven primären Speichereinheiten, erstellt ausschließlich primäre ARP-Snapshots und bietet auf maschinellem Lernen basierende Ransomware-Erkennung für diese Speichereinheiten. Um ARP/AI zu aktivieren, siehe link:../secure-data/enable-anti-ransomware-protection.html["Autonomen Ransomware-Schutz mit KI aktivieren"].
