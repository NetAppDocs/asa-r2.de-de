...
Ab ONTAP 9.18.1 kann der Snapshot-Reservierungsprozentsatz festgelegt und die automatische Snapshot-Löschung aktiviert werden, wenn eine Konsistenzgruppe mit neuen Speichereinheiten erstellt wird. Die Snapshot-Reserve ist der Speicherplatz in der Speichereinheit, der speziell für Snapshots reserviert ist. Wenn die Snapshot-Reserve zusammen mit der automatischen Snapshot-Löschung konfiguriert ist, werden ältere Snapshots automatisch gelöscht, sobald der von Snapshots belegte Speicherplatz die Snapshot-Reserve überschreitet. Wenn Snapshot-Reserve und automatische Snapshot-Löschung für eine übergeordnete Konsistenzgruppe aktiviert sind, sind sie auch für alle vorhandenen untergeordneten Konsistenzgruppen aktiviert. Wenn neue untergeordnete Konsistenzgruppen hinzugefügt werden, übernehmen sie die Einstellungen für Snapshot-Reserve und Snapshot-Löschung der übergeordneten Gruppe nicht.

link:snapshot-reserve.html["Erfahren Sie mehr über Snapshot-Reservierung auf ASA r2-Speichersystemen."].

Ab ONTAP 9.16.1 ist es möglich, beim Erstellen von Konsistenzgruppen mit neuen Speichereinheiten bis zu fünf untergeordnete Konsistenzgruppen zu konfigurieren. link:manage-hierarchical-consistency-groups.html["Erfahren Sie mehr über Kinderkonsistenzgruppen auf ASA r2-Systemen."].
...
====
.Was kommt als Nächstes?
Nachdem Ihre Daten nun durch Snapshots geschützt sind, empfiehlt es sich, link:snapshot-replication.html["Richten Sie die Snapshot-Replikation ein"] Ihre Konsistenzgruppen zur Datensicherung und Notfallwiederherstellung an einen geografisch entfernten Ort zu kopieren.
