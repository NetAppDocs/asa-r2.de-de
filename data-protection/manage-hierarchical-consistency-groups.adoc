...
summary: Konsistenzgruppen ermöglichen das gleichzeitige Ausführen verschiedener Aktionen auf mehreren Speichereinheiten, anstatt Aktionen einzeln auf jeder Einheit durchzuführen. In einer hierarchischen Struktur sind eine oder mehrere Konsistenzgruppen als untergeordnete Konsistenzgruppen einer übergeordneten Konsistenzgruppe konfiguriert. 
---
...
[role="lead"]
Ab ONTAP 9.16.1 kann der System Manager verwendet werden, um hierarchische Konsistenzgruppen auf Ihrem ASA r2 System zu erstellen. In einer hierarchischen Struktur werden eine oder mehrere Konsistenzgruppen als untergeordnete Gruppen unter einer übergeordneten Konsistenzgruppe konfiguriert. Für jede untergeordnete Konsistenzgruppe kann eine individuelle Snapshot-Richtlinie angewendet werden, und die Snapshots aller untergeordneten Konsistenzgruppen können als eine Einheit auf einen Remote-Cluster repliziert werden, indem die übergeordnete Konsistenzgruppe repliziert wird. Dadurch werden Datenschutz und Verwaltung für komplexe Datenstrukturen vereinfacht.
...
. Fahren Sie mit dem Mauszeiger über die Konsistenzgruppe, die in eine übergeordnete Konsistenzgruppe umgewandelt werden soll.
...
Sie können zusätzliche untergeordnete Konsistenzgruppen unter der übergeordneten Konsistenzgruppe erstellen. Es besteht außerdem die Möglichkeit, link:snapshot-replication.html["Richten Sie die Snapshot-Replikation ein"] die übergeordnete und die untergeordneten Konsistenzgruppen zur Datensicherung und Notfallwiederherstellung an einen geografisch entfernten Standort zu kopieren.
...
. Fahren Sie mit dem Mauszeiger über die untergeordnete Konsistenzgruppe, die abgetrennt werden soll.
