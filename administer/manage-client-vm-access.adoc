...
Wenn auf der Speicher-VM iSCSI-LIFs vorhanden sind, die nicht für automatisches LIF-Failover aktiviert sind, werden Ihre neu erstellten LIFs ebenfalls nicht für automatisches LIF-Failover aktiviert.  Wenn die automatische LIF-Failover-Funktion nicht aktiviert ist und ein Failover-Ereignis eintritt, werden Ihre iSCSI-LIFs nicht migriert.
