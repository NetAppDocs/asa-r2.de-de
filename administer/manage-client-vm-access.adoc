...
Bei einem Komponentenausfall kann ein LIF auf einen anderen physischen Port übergehen oder migriert werden, sodass die Netzwerkkommunikation nicht unterbrochen wird. Ab ONTAP 9.19.1 wird automatisches LIF-Failover standardmäßig für iSCSI-, NVMe/TCP- und Shared-IP-LIFs unterstützt. Wenn automatisches LIF-Failover aktiviert ist, wird das LIF im Falle eines Storage-Failovers automatisch von seinem Heimatknoten oder -port auf seinen HA-Partnerknoten oder -port und nach Abschluss des Failovers wieder zurück migriert. Oder sollte der Port fehlerhaft werden, wird das LIF automatisch auf einen funktionierenden Port seines aktuellen Heimatknotens und nach Wiederherstellung des ursprünglichen Ports wieder dorthin zurückmigriert.

In ONTAP 9.18.1 und früher wird das automatische LIF-Failover für NVMe/TCP-LIFs oder gemeinsam genutzte IP-LIFs nicht unterstützt. Wenn Sie ausschließlich iSCSI-LIFs erstellen, ist das automatische LIF-Failover standardmäßig aktiviert, es sei denn, die Storage-VM verfügt bereits über iSCSI-LIFs, bei denen das automatische LIF-Failover nicht aktiviert ist. Wenn auf der Storage-VM ausschließlich iSCSI-LIFs vorhanden sind, bei denen das automatische LIF-Failover nicht aktiviert ist, wird es auch für die neu erstellten LIFs nicht aktiviert. In solchen Fällen müssen zunächst die vorhandenen ausschließlich iSCSI-LIFs gelöscht und anschließend mit automatischem LIF-Failover neu erstellt werden. Weitere Informationen dazu link:https://docs.netapp.com/us-en/ontap/san-admin/delete-lif-san-environment-task.html["Löschen einer LIF in einer SAN-Umgebung"^].
...
In ONTAP 9.18.1 und früher ist das automatische LIF-Failover standardmäßig aktiviert, wenn iSCSI-only-LIFs erstellt werden, sofern alle anderen iSCSI-only-LIFs auf der Storage-VM ebenfalls für das automatische LIF-Failover aktiviert sind. Wenn auf der Storage-VM iSCSI-only-LIFs vorhanden sind, die nicht für das automatische LIF-Failover aktiviert sind, werden auch die neu erstellten LIFs nicht für das automatische LIF-Failover aktiviert.
