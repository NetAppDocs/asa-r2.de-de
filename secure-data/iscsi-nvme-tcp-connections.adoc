---
sidebar: sidebar 
permalink: secure-data/iscsi-nvme-tcp-connections.html 
keywords: asa r2, secure data, data security, encrypt, data at rest, data-at-rest, encryption, iscsi, nvme, tcp, ipsec, data-in-flight, ip security, secure connections, secure data connections, hardware offload 
summary: Wenn Sie auf Ihrem ASA r2-System das IP-Protokoll verwenden, können Sie die IP-Sicherheit konfigurieren, um Ihre Datensicherheit zu erhöhen. 
---
= IPsec und Hardware Offload Unterstützung für ASA r2 Storage-Systeme
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
Wenn Sie auf Ihrem ASA r2-System das IP-Protokoll verwenden, kann IP-Sicherheit (IPsec) konfiguriert werden, um die Datensicherheit zu erhöhen. IPsec ist ein Internetstandard, der Verschlüsselung von Daten während der Übertragung, Authentifizierung auf IP-Ebene für Endpunkte sowie Schutz vor Replay- und Man-in-the-Middle-Angriffen bietet.

Für ASA r2-Systeme ist IPsec für iSCSI- und NVMe/TCP-Hosts verfügbar.

Auf bestimmten ASA r2-Systemen können mehrere kryptografische Vorgänge, wie z. B. Verschlüsselungs- und Integritätsprüfungen, auf eine unterstützte NIC-Karte (Network Interface Controller) ausgelagert werden. Der Durchsatz für auf die NIC-Karte ausgelagerte Vorgänge beträgt etwa 5 % oder weniger. Dies kann die Leistung und den Durchsatz des durch IPsec geschützten Netzwerkverkehrs erheblich verbessern.

Ab ONTAP 9.18.1 wird die Unterstützung für IPsec Hardware-Offload auf IPv6-Datenverkehr ausgeweitet.

Die folgenden Netzwerkkarten werden für die Hardware-Auslagerung auf den folgenden ASA r2-Systemen und ONTAP Versionen unterstützt:

[cols="3"]
|===
| Unterstützte NIC-Karte | ASA r2-Systeme | ONTAP -Version 


 a| 
X50135A (2p, 40G/100G Ethernet-Controller)
 a| 
* ASA A1K
* ASA A90
* ASA A70

 a| 
ONTAP 9.17.1 und höher



 a| 
X60135A (2p, 40G/100G Ethernet-Controller)
 a| 
* ASA A50
* ASA A30
* ASA A20

 a| 
ONTAP 9.17.1 und höher



 a| 
X50131A – (2P, 40 G/100 G/200 G/400 G Ethernet-Controller)
 a| 
* ASA A1K
* ASA A90
* ASA A70

 a| 
ONTAP 9.16.1 und höher



 a| 
X60132A – (4p, 10G/25G Ethernet-Controller)
 a| 
* ASA A50
* ASA A30
* ASA A20

 a| 
ONTAP 9.16.1 und höher

|===
Siehe dielink:https://hwu.netapp.com/["NetApp Hardware Universe"^] Weitere Informationen zu den unterstützten Systemen und Karten finden Sie hier.

.Was kommt als Nächstes?
IPsec wird auf Ihrem ASA r2-System auf die gleiche Weise konfiguriert wie auf anderen ONTAP -Systemen. Weitere Informationen finden Sie unter link:https://docs.netapp.com/us-en/ontap/networking/ipsec-prepare.html["Bereiten Sie die Konfiguration der IP-Sicherheit für das ONTAP Netzwerk vor."^].
