...
Zwischen der ASA r2 REST API und der einheitlichen ONTAP REST API, die mit FAS, AFF und ASA Systemen verfügbar ist, bestehen einige Unterschiede. Das Verständnis dieser Änderungen ermöglicht eine effektivere Nutzung der Online API Referenzdokumentation.
...
Einige ASA r2 Endpunkt- und Methodenkombinationen unterstützen nicht alle für die einheitliche ONTAP Persönlichkeit definierten Eigenschaften. Beispielsweise werden bei Verwendung von PATCH mit dem Endpunkt `/api/storage/volumes/{uuid}` mehrere Eigenschaften von ASA r2 nicht unterstützt, darunter:
...
OAuth 2.0 ist das Branchenstandard-Autorisierungs-Framework. Es wird verwendet, um den Zugriff auf geschützte Ressourcen basierend auf signierten Zugriffstoken einzuschränken und zu kontrollieren. OAuth 2.0 kann mit System Manager konfiguriert werden, um die Systemressourcen von ASA r2 zu schützen.
