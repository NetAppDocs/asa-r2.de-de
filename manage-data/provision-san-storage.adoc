...
---
= ONTAP SAN Storage auf ASA r2 Systemen bereitstellen
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
Ab ONTAP 9.19.1 müssen Sie beim Erstellen einer AIX-LUN, die Sie für SnapMirror active sync-Schutz aktivieren möchten, unter *Hostinformationen* die Option *lokal* für den *Zugriffsmodus* auswählen. Dies ist erforderlich, um ein transparentes Anwendungs-Failover zu ermöglichen.
...
+
Der Host-NQN wird als <nqn.yyyy-mm> gefolgt von einer vollqualifizierten Domain formatiert. Das Jahr sollte gleich 1970 oder später sein. Die maximale Gesamtlänge sollte 223 betragen. Ein Beispiel für einen gültigen NVMe-Initiator ist nqn.2014-08.com.example:string.
