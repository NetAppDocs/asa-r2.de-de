...
| link:../data-protection/manage-consistency-groups.html["Unterstützung für Geometrieänderungen an Konsistenzgruppen in Replikationsbeziehungen"] | ASA r2-Systeme unterstützen Geometrieänderungen an Konsistenzgruppen in einer SnapMirror active sync- oder asynchronen Replizierungsbeziehung, ohne die SnapMirror active sync-Beziehung zu löschen oder die asynchrone Beziehung zu unterbrechen. Bei einer Geometrieänderung an der primären Konsistenzgruppe wird diese Änderung auf die sekundäre Konsistenzgruppe repliziert. 
...
| link:../administer/space-balancing.html["Unterstützung für automatischen Workload-Ausgleich"] | Um Leistung und Ressourcennutzung zu optimieren, werden Arbeitslasten automatisch zwischen den Knoten eines HA-Paares ausgeglichen. 
|===