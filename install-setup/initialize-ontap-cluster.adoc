...
In bestimmten Fällen müssen Sie möglicherweise link:../learn-more/cli-support.html#set-up-an-ontap-asa-r2-cluster-using-the-cli["Verwenden Sie die ONTAP -Befehlszeilenschnittstelle (CLI), um Ihren Cluster einzurichten."]Die Sie sollten die CLI beispielsweise dann verwenden, wenn Ihre Sicherheitsprotokolle es Ihnen nicht erlauben, einen Laptop an Ihre Management-Switches anzuschließen, oder wenn Sie ein Nicht-Windows-Betriebssystem verwenden.
