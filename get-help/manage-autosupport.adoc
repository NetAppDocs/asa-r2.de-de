...
[role="lead"]
AutoSupport überwacht proaktiv den Zustand Ihres Systems und sendet automatisch Meldungen an NetApp technischen Support, Ihre interne Supportorganisation und einen Supportpartner.

AutoSupport-Nachrichten an den technischen Support sind bei der Einrichtung Ihres Clusters standardmäßig aktiviert. Die korrekten Optionen müssen festgelegt werden und ein gültiger Mail-Host muss vorhanden sein, damit Nachrichten an Ihre interne Supportorganisation gesendet werden können. ONTAP beginnt 24 Stunden nach der Aktivierung mit dem Versenden von AutoSupport-Nachrichten.
...
. Die E-Mail-Adresse des Empfängers eingeben und anschließend die Empfängerkategorie auswählen.
+
Für Partner ist die Empfängerkategorie *Partner* auszuwählen. Für Mitglieder der internen Supportorganisation ist *Allgemein* auszuwählen.
...
.Was kommt als Nächstes?
AutoSupport-Fälle werden während des von Ihnen angegebenen Zeitraums nicht generiert. Wenn das Upgrade oder die Wartungsarbeiten vor Ablauf des angegebenen Zeitraums abgeschlossen werden, wird die Generierung von Supportfällen umgehend wieder aufgenommen.
